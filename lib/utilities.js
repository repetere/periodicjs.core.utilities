...
/**
 * simple helper function for encrypting a string
 * @param {string} password A string that will be used as the encryption key
 * @param {string} algorithm A string that defines the encryption algorithm to be used.  Alghorithm can be any available to node crypto package
 * @param {string} encryptable A string to encrypt
 * @return {string} encrypted string
 */
Utilities.prototype.encrypt = function (password, algorithm, encryptable) {
	var cipher = crypto.createCipher(algorithm, password),
		encrypted = cipher.update(encryptable, 'utf8', 'hex');
	encrypted += cipher.final('hex');
	return encrypted;
};

/**
 * simple helper function for decrypting a string
 * @param {string} password A string that represents a decryption key to be used.  The decryption key must match the encryption key
 * @param {string} algorithm A string that defines the decryption algorithm to be used.  The decryption algorithm must match the encryption algorithm
 * @param {string} decryptable An ecrypted string to decrypt
 * @return {string} decrypted string
 */
Utilities.prototype.decrypt = function (password, algorithm, decryptable) {
	var decipher = crypto.createDecipher(algorithm, password),
		decrypted = decipher.update(decryptable, 'hex', 'utf8');
...
Utilities.prototype.removeEmptyObjectValues = function (obj) {
	for (var property in obj) {
		if (typeof obj[property] === 'object') {
			...
			if (obj[property] === '' || obj[property] === ' ' || obj[property] === null || obj[property] === undefined || Object.keys(obj).length === 0 || (Array.isArray(obj) && obj.length===0)) {
				delete obj[property];
			}
		}
	}
...
Utilities.prototype.getAdminMenu = function (options) {
	...
	for (var x in appSettings.extconf.extensions) {
		if (appSettings.extconf.extensions[x].enabled === true && (appSettings.extconf.extensions[x].periodicConfig['periodicjs.ext.asyncadmin'] || appSettings.extconf.extensions[x].periodicConfig['periodicjs.ext.admin'])) {
			var extmenudata = appSettings.extconf.extensions[x].periodicConfig['periodicjs.ext.asyncadmin'] || appSettings.extconf.extensions[x].periodicConfig['periodicjs.ext.admin'];
